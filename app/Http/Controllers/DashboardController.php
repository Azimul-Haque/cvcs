                ...
                <div class="modal fade" id="sendToDefectiveListModal'. $row->member_id .'" role="dialog">
                  <div class="modal-dialog modal-md">
                    <div class="modal-content">
                      ...
                      <form method="POST" class="form-default" action="' . route('dashboard.transfermember', $row->member_id) . '">
                      <input name="_token" type="hidden" value="24m1HlYKkpsbxLeo8NUygnlsDmUL0tBgNC1AFSWj">
                      <div class="modal-body">
                        <select name="branch_id" id="branch_id" class="form-control" required="">
                            <option value="" selected="" disabled="">দপ্তরের নাম নির্ধারণ করুন</option>
                            @foreach($branches as $branch)
                              <option value="' . $row->branch_id . '" @if($branch->id == $member->branch_id) selected="" @endif>{{ $branch->name }}</option>
                            @endforeach
                        </select><br/>
                        <div class="checkbox">
