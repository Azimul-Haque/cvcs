...
@section('body')
    <div class="login-box">
        ...
        <div class="login-box-body">
            <p class="login-box-msg">পাসওয়ার্ড পরিবর্তনকরন {{-- {{ trans('adminlte::adminlte.password_reset_message') }} --}}</p>
            @if (session('status'))
            ...
            <form action="{{ url(config('adminlte.password_email_url', 'password/email')) }}" method="post">
                ...
                >পাসওয়ার্ড রিসেট লিঙ্ক পাঠান{{-- {{ trans('adminlte::adminlte.send_password_reset_link') }} --}}</button>
            </form>
        </div>
        <!-- /.login-box-body -->
    </div><!-- /.login-box -->
